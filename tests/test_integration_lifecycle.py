...
def test_setup_entry_starts_client_forwards_platforms_and_fires_new_device(
    ...
    assert client.kwargs["host"] == "192.0.2.1"
    assert client.kwargs["username"] == "admin"
...
@pytest.mark.parametrize(
    "entry_data",
    [
        {"host": "127.0.0.1", "username": "admin", "password": "secret", "ssl": False},
        {"host": "192.0.2.1", "username": "admin", "password": "secret", "ssl": True},
    ],
)
...
def test_setup_entry_rejects_unrecoverable_entry_without_host() -> None:
    ...
    hass = _hass()
    entry = _FakeEntry({"username": "admin", "password": "secret"})
...
def test_unload_entry_clears_http_repair_issue_only_after_platform_unload(
    ...
    hass = _hass()
    entry = _FakeEntry({"host": "192.0.2.1", "username": "admin", "password": "secret"})
